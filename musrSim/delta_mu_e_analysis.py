...
def Plot_delta(d):
    ...
    for i in range(len(d)):
        ...
        #c7.Draw()
        c7.SaveAs(f"delta_d={int(d[i])}mm.pdf")


def file_delta_save(d):
    with open('data/delta_mu_e.dat','w') as f:
        f.write('# d, delta_mu_mean, delta_mu_std, delta_e_mean, delta_e_std, delta_mu_vx_mean, delta_mu_vx_std, delta_e_vx_mean, delta_e_vx_std, N_mu, N_e, N_vx_mu_e\n')
        f.write('# d & deltas are in mm. delta_e_vx is obtained from asking L1&L2 muon and L3&L4 positron in one event\n')
        for i in range(len(d)):
            print(f'{d[i]}')
            Targ= {'diameter':20.,'thickness':1.0,'z':0.} # Target/Sample diameter and thickness (of cylinder)    
            Detect = {
            ...
            f.write(f'{d[i]:.3f},{delta_mu_mean:.3f},{delta_mu_std:.3f},{delta_e_mean:.3f},{delta_e_std:.3f},{delta_mu_vx_mean:.3f},{delta_mu_vx_std:.3f},{delta_e_vx_mean:.3f},{delta_e_vx_std:.3f},{n_mu},{n_e},{n_mu_e_vx}\n')
...
plt.plot(d,delta_mu[:,1],'ok',lw=2)
plt.plot(d,delta_e[:,1],'or',lw=2)

plt.plot(d,delta_mu_vx[:,1],'ok',lw=2)
plt.plot(d,delta_e_vx[:,1],'or',lw=2)

for i in range(len(NN[:,0])):
    print(f'd={d[i]}')
    print(f'upstream mu+ efficiency  (stop in target)={100*NN[i,0]/N_events}%')
    print(f'downstream e+ efficiency (stop in target)={100*NN[i,1]/N_events}%')
    print(f'L1&L2 mu+ & L3&L4 e+ efficiency  (stop in target)={100*NN[i,2]/N_events}%')
